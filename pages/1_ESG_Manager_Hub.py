...
st.subheader("Analyze SME lenders, evaluate risk, and generate instant auditable reports.")
st.html("<hr>")
...
if search_btn and search_field.strip():
    ...
    if search_results:
        for i, (id, name, email, location, business_type, business_stage, loan_goal, sales, income_freqruency, 
                has_employees, essay, timestamp, bank_num, valid_id, business_permit, bank_statement,
                income_tax_file, collateral) in enumerate(search_results, start=1):
    ...
    if submissions:
        for i, (id, name, email, location, business_type, business_stage, loan_goal, sales, income_freqruency, 
                has_employees, essay, timestamp, bank_num, valid_id, business_permit, bank_statement,
                income_tax_file, collateral) in enumerate(submissions, start=1):
            col1, col2 = st.columns(2)
            col1.markdown(f"""
            Loan Application ID: {id}\n
            *{name}*  
